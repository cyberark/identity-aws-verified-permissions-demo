...
def lambda_handler(event, context) -> Dict:
    ...
    effect = check_authorization(
        principal_id=claims["sub"], action=method, resource=resource, claims=claims, user_attributes=user_attributes
    )
...
def _get_identity_tanant_public_key(token: str, identity_public_key_url: str) -> jwk.Key:
    ...
    response = requests.get(
        url=identity_public_key_url, headers={"Authorization": f"Bearer {token}"}, timeout=60
    )  # it is advised to cache the key results
...
def check_authorization(principal_id: str, action: str, resource: str, claims: Dict, user_attributes: Dict) -> str:
    ...
    context = {
        "context": {
            "aws_region": {"string": claims["aws_region"]},
            "last_login_time": {"long": int(claims["last_login"])},
            "login_time": {"long": int(datetime.now(timezone.utc).timestamp())},
            "weekday": {"long": datetime.now(timezone.utc).weekday()},
        }
    }
